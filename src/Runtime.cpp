...
{ /* Add your code here */
    Address Addr(R);
    SI.getMemory().insert(make_pair(Addr,SI.getContext().int_val((uintptr_t)Ptr)));
}
...
{ /* Add your code here */
    Address Addr(Y);
    SI.getMemory().insert(make_pair(Addr,SI.getContext().bv_val((uintptr_t)X, sizeof (X))));
}
...
{ /* Add your code here */
    ...
    if(Op == CmpInst::Predicate::ICMP_EQ)
        SI.getMemory().insert(make_pair(addr,SI.getContext().bool_val(X1 == X2)));
...
{ /* Add your code here */
    ...
    if(Op == Instruction::Add)
        SI.getMemory().insert(make_pair(addr,SI.getContext().bv_val(X1 + X2, sizeof (X1 + X2))));
